...
## 4. Data Dictionary (Gold Layer)

The following schema maps the structural table layout configured in the **AWS Glue Data Catalog** and accessed natively by **Amazon Athena** for production queries:

| Column Name | Data Type | Constraint / Description |
| :--- | :--- | :--- |
| `transaction_id` | `VARCHAR` | Unique surrogate key generated via alphanumeric hash combinations of the asset code and execution day. |
| `process_date` | `TIMESTAMP` | Systemic timestamp representing the precise closure date and time boundary of the ingested Binance candle. |
| `btc_close_price` | `DECIMAL(10, 2)` | The definitive closing price of Bitcoin at the session settlement mark, stored with high-precision decimals. |
| `rsi_14_value` | `DECIMAL(5, 2)` | Mathematical score generated by the Relative Strength Index model over a trailing 14-candle window. |
| `macd_signal_alignment` | `VARCHAR` | Current status indicator describing MACD positioning relative to its baseline signal (`BULLISH_CROSS`, `BEARISH_CROSS`, `NEUTRAL`). |
| `algo_recommendation` | `VARCHAR` | Final actionable signal delivered by the triple-confirmation script: `STRONG_BUY`, `SELL_PARTIAL`, `SELL_TOTAL`, or `HOLD_LIQUIDITY`. |
